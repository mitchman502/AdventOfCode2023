public class Advent3 {
    ...
    private static int part1(String prev, String cur, String next) {
        int rowVal = 0;

        char[] curChars = cur.toCharArray();
        for (int i=0; i<curChars.length; i++) {
            if (Character.isDigit(curChars[i])) {
                if (isSymbolAdjacent(prev, i) || isSymbolAdjacent(cur, i) || isSymbolAdjacent(next, i)) {
                    // System.out.println(curChars[i]);

                    int x,y;
                    for (x=i; x>=0; x--) {
                        if (!Character.isDigit(curChars[x])) break;
                    }
                    for (y=i; y<curChars.length; y++) {
                        if (!Character.isDigit(curChars[y])) break;
                    }
                    x++;

                    // System.out.println(x + " " + y);
                    // System.out.println(cur.substring(x, y));
                    rowVal += Integer.parseInt(cur.substring(x, y));
    ...
    private static boolean isSymbolAdjacent(String line, int curIndex) {
        if (line == null) {
            return false;
        }
        ...
        if (curIndex-1 != -1         && arr[curIndex-1] != '.' && Character.toString(arr[curIndex-1]).matches("[^0-9]")) return true;
        if (                            arr[curIndex  ] != '.' && Character.toString(arr[curIndex  ]).matches("[^0-9]")) return true;
        if (curIndex+1 != arr.length && arr[curIndex+1] != '.' && Character.toString(arr[curIndex+1]).matches("[^0-9]")) return true;
    ...
    private static int part2(String prev, String cur, String next) {
        ...
        for (int i=0; i<curChars.length; i++) {
            if (curChars[i] == '*') {
                int numUp   = numAdjacent(prev, i);
                int numSide = numAdjacent(cur, i);
    ...
    private static int numAdjacent(String line, int curIndex) {
        if (line == null) {
            return 0;
        }
        ...
        if (curIndex-1 != -1         && Character.isDigit(arr[curIndex-1])) numAdjacent++;
        if (                            Character.isDigit(arr[curIndex  ])) return 1;
        if (curIndex+1 != arr.length && Character.isDigit(arr[curIndex+1])) numAdjacent++;

        return numAdjacent;
    }

    private static int getNumVal(String line, int curIndex, int numHas) {
        if (line == null || numHas == 0) {
            return 1;
        }

        char[] arr = line.toCharArray();
        int[] vals = new int[numHas];
        int x = -100;
        int y = -100;

        if (numHas == 2) {
            for (x=curIndex-1; x>=0; x--) {
                if (!Character.isDigit(arr[x])) break;
            }
            for (y=curIndex-1; y<arr.length; y++) {
                if (!Character.isDigit(arr[y])) break;
            }
            x++;

            vals[1] = Integer.parseInt(line.substring(x, y));

            x = curIndex + 1;
            y = curIndex + 1;
        }
        else {
            for (int pos=curIndex-1; pos<=arr.length; pos++) {
                if (pos > -1 && Character.isDigit(arr[pos])) {
                    x = pos;
                    y = pos;
                    break;
                }
        ...
        vals[0] = Integer.parseInt(line.substring(x, y));
