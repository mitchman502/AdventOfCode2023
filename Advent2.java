public class Advent2 {
	...
	private static void part1() {
		String [] in = getInput();
		int possbleGameTally = 0;
		for (int i=1; i<=in.length; i++) {
		    if (isPossible(in[i-1])) {
		        possbleGameTally += i;
		    }
		}
		System.out.println("Possible game tally: " + possbleGameTally);
	}
	...
	private static boolean isPossible(String line) {
	    ...
	    for (String g : games) {
	        ...
	        if (g.charAt(0) == 'G') continue;
	        String [] vals = g.split(", ");
	        for (String numString : vals) {
	            int num = Integer.parseInt(numString.replaceAll("[^0-9]", ""));
	            if (numString.contains("red") && num > MAX_RED) {
	                // System.out.println("### Red too high: " + num + " ###");
	                return false;
	            }
	            if (numString.contains("green") && num > MAX_GREEN) {
	                // System.out.println("### Green too high: " + num + " ###");
	                return false;
	            }
	            if (numString.contains("blue") && num > MAX_BLUE) {
	                // System.out.println("### Blue too high: " + num + " ###");
	                return false;
	            }
	        }
	    }
	...
	private static void part2() {
		...
		int powerSum = 0;
		for (String str : in) {
		    powerSum += gameValue(str);
		}
		System.out.println("powerSum: " + powerSum);
	}
	
	private static int gameValue(String line) {
	    int red = 0;
	    int green = 0;
	    int blue = 0;
	    line = line.replace(":", ";");
	    ...
	    for (String g : games) {
	        ...
	        for (String numString : vals) {
	            int num = Integer.parseInt(numString.replaceAll("[^0-9]", ""));
	            if (numString.contains("red") && num > red) {
	                red = num;
	            }
	            if (numString.contains("green") && num > green) {
	                green = num;
	            }
	            if (numString.contains("blue") && num > blue) {
	                blue = num;
	            }
	        }
	    }
